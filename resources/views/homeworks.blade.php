...
<div class="row mx-5 row-cols-1 row-cols-md-2 row-cols-lg-3" style="margin-top:100px">
    @foreach($homeworks as $homework)
        ...
        ?>
        @if(strtotime($homework['deadline'])>=strtotime(date("Y-m-d")) && !$homework['isDone'])
            ...
            <div class="card shadow mb-5 p-0">
                ...
                <div class="card-body px-4 py-4">
                    ...
                    <div class="progress my-4">
                        @if($diff->days>1)
                        <div class="progress-bar" style={{"width:".($homework['done']*100/$student_num)."%"}}></div>
                        @else
                        <div class="progress-bar bg-danger" style={{"width:".($homework['done']*100/$student_num)."%"}}></div>
                        @endif
                    </div>
                    <div>
                        @if($diff->days>1)
                        <p class="text-center m-0"><a class="btn btn-primary px-4" onclick="showConfirmation()" href="#" type="button">クリア</a></p>
                        @else
                        <p class="text-center m-0"><a class="btn btn-danger px-4" onclick="showConfirmation()" href="#" type="button">クリア</a></p>
                        @endif
                    </div>
                </div>
                <script>
